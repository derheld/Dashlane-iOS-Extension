...
- (instancetype)initWithAppName:(NSString *)appName presentFrom:(UIViewController *)vc sender:(id)sender
{
	self = [super init];
...
- (void)sendRequestWithCompletionBlock:(RequestCompletionBlock)completionBlock
{
	...
	if (extensionItem){
		...
		if ([self.sender isKindOfClass:[UIBarButtonItem class]]) {
			activityController.popoverPresentationController.barButtonItem = self.sender;
		}
